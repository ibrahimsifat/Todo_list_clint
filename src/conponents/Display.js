...
export default function Display() {
  //   console.log(JSON.parse(Cookies.get("IB_TODO")));
  ...
  return (
    <div>
      <div className="bg-slate-100 h-full pb-20 min-h-screen ">
        ...
        <div className="flex mx-auto   flex-col pt-28 items-center justify-center sm:w-10/12 w-11/12">
          ...
          {todos.map((todo) => (
            <TodoList todo={todo} handleDelete={handleDelete} key={todo._id} />
          ))}
          <p className="absolute hidden md:block bottom-14 right-14 bg-yellow-400 py-2 px-4 rounded-full text-lg font-bold">
            ?
          </p>
        </div>
      </div>
